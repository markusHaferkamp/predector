...
-n|--name         -- For conda, sets the environment name (default: 'predector').
                     For docker, sets the image tag (default: 'predector/predector:1.2.8-alpha').
...
curl -o environment.yml https://raw.githubusercontent.com/ccdmb/predector/1.2.8-alpha/environment.yml
...
```bash
curl -s https://raw.githubusercontent.com/ccdmb/predector/1.2.8-alpha/Dockerfile \
| docker build \
  ...
  -t predector/predector:1.2.8-alpha \
  -f - \
  .
```

Your container should now be available as `predector/predector:1.2.8-alpha` in your docker registry `docker images`.
...
curl -o ./singularity.def https://raw.githubusercontent.com/ccdmb/predector/1.2.8-alpha/singularity.def
...
```bash
singularity build predector.sif docker-daemon://predector/predector:1.2.8-alpha
```
...
docker save predector/predector:1.2.8-alpha > predector.tar
```
